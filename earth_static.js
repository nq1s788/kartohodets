...
function init() {
    ...
    sun.position.set(-5, 0, -3); // спереди по камере
    sun.target.position.set(0, 0, 0); // Земля по центру
    scene.add(sun);
    scene.add(sun.target);

    const textureLoader = new THREE.TextureLoader();

    const dayTexture = textureLoader.load('res/earth_day.jpg');
    dayTexture.colorSpace = THREE.SRGBColorSpace;
    dayTexture.anisotropy = 8;

    const nightTexture = textureLoader.load('res/earth_night.jpg');
    nightTexture.colorSpace = THREE.SRGBColorSpace;
    nightTexture.anisotropy = 8;

    const bumpRoughnessCloudsTexture = textureLoader.load('res/earth_bump.jpg');
    ...
    const atmosphereColor = mix(color('#bc430b'), color('#4db2ff'), sunOrientation.smoothstep(-0.45, 0.75));
